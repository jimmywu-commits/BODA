/**
 * 商品建議售價更新器（即時寫入 F 欄＋右側進度視窗）
 *
 * C 欄：銷售狀態；包含「已售出」時跳過，不查詢、不修改 F 欄
 * D 欄：商品狀況（全新／二手）
 * E 欄：品名
 * F 欄：建議售價
 *
 * 請將本檔案全部貼到綁定在 Google Sheet 的 Apps Script 專案中。
 * 不需要另外建立 HTML 檔案。
 */
...
function runPriceUpdate_() {
  ...
  try {
    ...
    // 一次讀取 C:E：C 狀態、D 狀況、E 品名。
    const inputs = sheet
      .getRange(CONFIG.START_ROW, CONFIG.SOLD_STATUS_COL, rowCount, 3)
      .getDisplayValues();
    const memo = {};

    inputs.forEach(function (row, index) {
      const sheetRow = CONFIG.START_ROW + index;
      const soldStatus = String(row[0] || '').trim();
      const conditionText = String(row[1] || '').trim();
      const productName = String(row[2] || '').trim();
      const priceCell = sheet.getRange(sheetRow, CONFIG.PRICE_COL);

      // C 欄只要包含「已售出」，就跳過且不修改 F 欄。
      if (/已售出/.test(soldStatus)) {
        progress.skipped++;
        progress.processed = index + 1;
        progress.currentRow = sheetRow;
        progress.currentProduct = productName || '(已售出)';
        progress.message = '略過第 ' + sheetRow + ' 列：C 欄已售出。';
        writeStatus_(progress);
        return;
      }
      ...
      progress.message = '正在查詢第 ' + sheetRow + ' 列：' + (productName || '(沒有品名)');
      writeStatus_(progress);

      if (!productName) {
        progress.skipped++;
        progress.processed = index + 1;
        priceCell.setNote('未更新：E 欄沒有品名。');
      ...
      if (!type) {
        progress.skipped++;
        progress.processed = index + 1;
        priceCell.setNote('未更新：D 欄請填「全新」或「二手」。目前內容：' + conditionText);
      ...
      try {
        ...
        // 重要：每筆成功結果立即寫入 F 欄，不等整批完成。
        priceCell
          .setValue(memo[cacheKey].price)
          .setNote(makeNote_(memo[cacheKey], conditionText, productName));
        SpreadsheetApp.flush();

        progress.updated++;
        progress.message = '完成第 ' + sheetRow + ' 列：NT$' + formatNumber_(memo[cacheKey].price);
      } catch (error) {
        // 失敗時不清除 F 欄原值，只留下錯誤備註。
        progress.failed++;
        priceCell.setNote(makeErrorNote_(conditionText, productName, error));
        progress.message = '第 ' + sheetRow + ' 列查詢失敗：' + (error.message || error);
      }
    ...
    progress.message = '更新完成。成功 ' + progress.updated + ' 筆，略過 ' + progress.skipped + ' 筆，失敗 ' + progress.failed + ' 筆。';
    writeStatus_(progress);
    return progress;
  } catch (error) {
